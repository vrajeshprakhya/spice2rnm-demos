...
say "tuning-curve check scores them identically -- it passes both. Fed a"
say "control with a 100 ps correlation time, the sampling one produced"
say "3.37x the transistors\' period jitter."
echo
say "That gap is not a fixed property of the two forms. It is"
say "sqrt(T_half/tau) in the correlation time of the disturbance --"
say "sqrt(1250/100) = 3.54 here, which is the 3.37 above to within 5%."
say "A faster disturbance separates them further, so a ratio quoted"
say "without its tau says less than it looks like it does."
echo
