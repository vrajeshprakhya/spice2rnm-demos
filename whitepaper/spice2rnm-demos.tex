...
classifies them from the measured DC curve --- $62\%$ of swept points
within $5\%$ of a rail is a switching characteristic, not a smooth
response --- and scores them on duty cycle instead.
...
its voltage band: both inverter inputs return $399.8\unit{MHz}$, the
$400\unit{MHz}$ this PLL locks to. Scored there, they pass at
$0.276$ and $0.307\unit{pp}$ of duty against a $0.5\unit{pp}$ bar.

That number is not one a language model can supply. Asked four times
about the same block with the same measurements, the classifier proposed
clock rates of $2\unit{MHz}$, $765\unit{MHz}$, $300\unit{MHz}$ and
$10\unit{GHz}$ --- the first two hundred times below where the buffer
runs, the last twenty-five times above what its own model can follow. The
measurement returns the same answer every run, and gives both stages the
same answer, which no classifier run did. Intent from the model, physics
from the circuit, applied to a number that looked like intent and was not.
...
$19$ minutes, most of it the ring oscillator's tuning curve.
