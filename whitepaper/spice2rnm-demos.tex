...
$10.91\unit{ps}$ rms of period jitter at the oscillator --- $\pm 5\%$ at
the two hundred cycles it was measured over, and itself a draw --- and
a constant rail produces none of it.
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.3em]
  ...
  \item \textbf{Jitter TRANSFER is verified; intrinsic device noise is
        ...
        The distinction, and the way each is implemented, are Kundert's:
        \emph{Modeling and Simulation of Jitter in Phase-Locked Loops},
        AACD 1997. He separates the non-accumulating jitter of driven
        systems from the autonomous kind, whose phase difference is ``a
        random walk that is not bounded''; takes $J = \sigma(T)$, the
        standard deviation of one period, as the metric for both; models
        driven jitter by ``randomly dithering the delay argument \ldots{}
        on every output transition''; and scales the per-update dither by
        $\sqrt{N}$ for $N$ updates per period, giving $1.414$ for the two
        this model uses.

        The emitted model was checked against all three. Asked for $1\%$
        rms period jitter it produced $0.9998\%$; its scaling constant is
        the $\sqrt{2}$ his listing specifies; and over $30{,}000$ cycles
        the variance of the accumulated deviation grew linearly in cycle
        count at $1.004 \pm 0.026$ of the $k J^2$ his random-walk
        description requires. That check also found a defect: at one
        sub-step per half cycle the scheduler cannot resolve a randomised
        threshold, and the model now refuses that combination rather than
        reporting $39.6\%$ jitter where $1\%$ was asked.} The
