...
\begin{itemize}[leftmargin=1.5em,itemsep=0.3em]
  ...
  \item \textbf{Noise and jitter are not verified}, and the reason is
        worth stating rather than leaving to be discovered. A real number
        model is deterministic, and the reference runs are SPICE
        transients with no noise sources, so neither side of the
        comparison carries noise: checking one against the other would
        produce a confident zero that means nothing. Device and phase
        noise need \texttt{.noise} or transient-noise analysis on the
        reference side and a model built to carry a noise source, which
        is a different kind of model from the one this tool emits.
        ...
        This is where the published practice sits too. Analog Devices'
