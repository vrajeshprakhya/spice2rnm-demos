...
\quad the same loop, no co-simulation & pure-SV loop, declared bands & $5 + 1$ & $0$ \\
\bottomrule
\end{tabular}
\end{center}
...
closing the loop and running it is $42.7$ seconds. Its extra check is
the loop reaching the frequency the reference and the divider imply, and
its $5$ per-block passes are worth less than the row above them, for the
reason given in Section~\ref{sec:nocosim}.
...
question asked is whether the assembled thing does what it is for.
Section~\ref{sec:nocosim} runs the same PLL that way and it locks to
$+0.0004\%$ of its reference. That is a weaker claim than the second
check makes, deliberately: there are no transistors in it to disagree
with.
