...
else --- on this netlist, \texttt{clk}. The output is a guess from a
node's name, because a netlist never says which node you care about, and
the run reports it as a guess so a reader can see which it was. Four
routes refuse to guess at all and ask to be told: the ones that measure a
specific node's edge, frequency or current.
