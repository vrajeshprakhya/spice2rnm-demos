...
spice2rnm-demos/
  ...
  showcase/          the generated artifacts, byte-for-byte
    ...
    pll/             11 files: five block models, three
                     interfaces, the composed analog top, the
                     node package and result.json
...
to be a waveform is a trace. Selecting these folders by file extension
instead published nothing at all for two of the five routes --- the RC
network and the phase sweep produce no log file, only a table --- which
is the kind of gap a directory listing hides, because a folder holding a
manifest and no evidence still looks populated.
