...
\quad \texttt{vout} (control voltage) & $1.83\unit{mV}$, bar $0.174$ \\
\quad \texttt{aout} (VCO output)      & $0.4\%$ of crossings, bar $2\%$ \\
\quad \texttt{dra} (internal probe)   & $9.31\unit{mV}$ \\
\midrule
Both loops lock at                   & $400.00\unit{MHz}$ \\
\quad settled control band           & $1.8468$--$1.8546\unit{V}$ vs \\
                                     & $1.8468$--$1.8499\unit{V}$ \\
\midrule
Model half vs transistor half        & $6.10\unit{s}$ vs $101.3\unit{s}$ \\
\quad same testbench, same span       & $16.6\times$ \\
\bottomrule
\end{tabular}
\end{center}
...
first --- and the two agree to $1.83\unit{mV}$ while reaching the same
...
the run before this one gave $3.61\unit{mV}$ here and $17.7\unit{mV}$ on
\texttt{dra}, about twice these. What does not move is what the
difference is judged against --- $0.174$ of \texttt{vout}'s own scale,
of which $0.124$ is the allowance for exactly this sampling --- and that
is the number the verdict is against and the record carries. A single
...
shipping. The deletion is worth $12.22\unit{ps}$ rms of period jitter on
this ring, which its supply moves at $450.6\unit{MHz/V}$.
...
\begin{tabular}{lrrr}
\hline
\textbf{Injected} & \textbf{Circuit} & \textbf{Model} & \textbf{Ratio}\\
\hline
none (floor)        & $0.049\unit{ps}$ & $0.000\unit{ps}$ & not judged\\
$5\unit{mV}$ rms    & $2.45\unit{ps}$  & $2.49\unit{ps}$  & $1.02$\\
$20\unit{mV}$ rms   & $10.74\unit{ps}$ & $10.27\unit{ps}$ & $0.96$\\
$50\unit{mV}$ rms   & $23.90\unit{ps}$ & $23.72\unit{ps}$ & $0.99$\\
\hline
\end{tabular}
\end{center}

The floor row is measured and deliberately not judged. With nothing
injected the reference still reports about $50\unit{fs}$, which is how
...
The agreement is not a fit. A static sensitivity of $450.6\unit{MHz/V}$
on a $2.5\unit{ns}$ period predicts $14.1\unit{ps}$ of period jitter for
$5\unit{mV}$ rms, if the disturbance held still across the cycle. The
circuit gives the $2.45\unit{ps}$ in the table above --- a ratio of
$0.174$ against the $\sqrt{100\unit{ps}/2500\unit{ps}} = 0.200$ that
averaging over the cycle predicts. It holds across the decade, not at one
point: $0.174$, $0.191$ and $0.170$ at the three judged amplitudes, where
an oscillator that did not average its input would sit at $1$. The model
reproduces that because it integrates, and the version that samples does
not.

\textbf{And it decides for itself whether the check is worth running.}
The honest answer for most designs is no, and the tool says so with
numbers rather than skipping in silence. On the reference PLL the control
moves $12.18\unit{\textmu V}$ rms within one $2.503\unit{ns}$ period ---
worth $0.0288\unit{ps}$ of movement against a $0.0521\unit{ps}$
measurement floor, which is $0.55\times$ what the reference can resolve.
There the two model forms agree to less than the measurement can see, so a
sweep would report the solver and not the circuit, and the run records
that reasoning instead of a verdict. The check runs when the design's own
closed-loop behaviour says it would measure something, and when the deck
handed to the block declares transient noise of its own. There is no flag
to force it, because a request to run a check is not evidence that one is
warranted; and where the tool has no closed-loop evidence either way it
runs the check rather than skipping it. An absent judgement is not a
judgement to skip on.

\paragraph{Where the four rows above come from.} Not from this PLL, and
saying so is cheaper than letting a reader find it. The gate declines
here --- the figures just quoted are exactly why --- so the sweep was run
by calling it directly rather than through the pipeline's own decision.
Two consequences a reader is owed. The deck for this case \emph{does}
declare $20\unit{mV}$ rms on its supply, and that condition reaches the
composed model (Section~\ref{sec:noise}), but it does not reach this
gate: each block is characterised from a standalone fragment carrying its
own bias sources, not the top-level rail, so the declaration is invisible
where the decision is made. And the sweep is reachable only as a library
call, with no command-line flag behind it, so no run that produces the
rest of this paper exercises it.
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.3em]
  ...
  \item \textbf{Jitter TRANSFER is verified; intrinsic device noise is
        not.} The distinction matters, so both halves are stated.
        ...
        \ref{sec:jitter} gives the measurements. This is a real check
        with a verdict, and it fails a model whose transfer is wrong.

        What is \emph{not} checked is the oscillator's own noise. SPICE
        simulates no device noise in a transient, so a plain transient
        shows noise only where the netlist declares a source --- which
        means a model emitting no intrinsic jitter matches what the
        reference run shows while matching the circuit less well. Getting
        at that needs the impulse sensitivity function, and the tool now
        measures one: on the reference ring
        $\Gamma_{\mathrm{rms}} = 4.33\times10^{12}\unit{rad/C}$, stable to
        $1.7\%$ across a five-fold change in injected charge and linear to
        $1\%$ at the phase where the oscillator is most sensitive.\footnote{%
        The method is Hajimiri, Limotyrakis and Lee, \emph{Jitter and
        Phase Noise in Ring Oscillators}, IEEE JSSC 34(6), June 1999:
        inject a narrow current pulse at each point in the cycle and
        measure the lasting phase shift. Two details decided whether it
        worked here. The pulse must be applied differentially where the
        circuit is differential --- into one node of a differential ring
        it is mostly common mode, which the ring rejects by design, and
        the measurement then reads its own floor no matter how large the
        pulse. And linearity has to be judged at the phase of greatest
        sensitivity, not a convenient one: the same charge that moves the
        phase by $0.04\unit{ps}$ at an insensitive point moves it by
        $52\unit{ps}$ at the peak.}
        That number is the quantity which converts a device noise power
        into a jitter. Turning it into a model that emits its own jitter
        is not done, and until it is, intrinsic jitter is measured but not
        modelled.
