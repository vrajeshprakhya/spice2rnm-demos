...
circuit. On \texttt{vout} that allowance is $0.127$ of the $0.177$ bar,
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.3em]
  ...
  \item \textbf{Jitter TRANSFER is verified; intrinsic device noise is
        ...
        traversal draws independently and nothing accumulates. The
        ...
        It is supplied rather than measured --- a transient of a deck
        declaring no noise has no delay variation in it to measure --- and
        the generated file states that, along with the constant latency
        the perturbation costs: a tap only looks backwards, so an edge can
        be early only relative to a delay already present.
        ...
        that environment on the composed PLL gives $12.68\unit{ps}$ rms
        of period jitter over $1151$ periods at the boundary, beside
        $5$ system checks with none failed and one inconclusive, over
        $116{,}012$ boundary crossings.

        REPORTED, NOT JUDGED, and the reason is not modesty. A loop
        ...
        and the two are not expected to agree. The $12.31\unit{ps}$ this
        deck's declared rail is worth at \texttt{ro\_vco} is a different
        measurement of a different quantity; that the two land within
        $3\%$ is worth recording and is not evidence of either.
