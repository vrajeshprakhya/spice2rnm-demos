...
\quad \texttt{vout} (control voltage) & $4.15\unit{mV}$, bar $0.262$ \\
\quad \texttt{aout} (VCO output)      & $0.4\%$ of crossings, bar $2\%$ \\
\quad \texttt{dra} (internal probe)   & $19.9\unit{mV}$ \\
\midrule
Both loops lock at                   & $400.00\unit{MHz}$ \\
\quad settled control band           & $1.8468$--$1.8546\unit{V}$ vs \\
                                     & $1.8468$--$1.8499\unit{V}$ \\
\midrule
Model half vs transistor half        & $7.09\unit{s}$ vs $119.3\unit{s}$ \\
\quad same testbench, same span       & $16.8\times$ \\
\bottomrule
\end{tabular}
\end{center}
...
first --- and the two agree to $4.15\unit{mV}$ while reaching the same
...
three consecutive runs of this same deck gave $3.61$, $1.83$ and
$4.15\unit{mV}$ here. What does not move is what the difference is judged
against --- $0.262$ of \texttt{vout}'s own scale, of which $0.212$ is the
...
\hline
either  & none (floor)      & $0.050\unit{ps}$  & $0.000\unit{ps}$  & not judged\\
\hline
control & $5\unit{mV}$ rms  & $2.327\unit{ps}$  & $2.394\unit{ps}$  & $1.029$\\
        & $20\unit{mV}$ rms & $8.952\unit{ps}$  & $9.169\unit{ps}$  & $1.024$\\
        & $50\unit{mV}$ rms & $23.314\unit{ps}$ & $22.832\unit{ps}$ & $0.979$\\
\hline
supply  & $5\unit{mV}$ rms  & $2.861\unit{ps}$  & $2.842\unit{ps}$  & $0.993$\\
        & $20\unit{mV}$ rms & $11.642\unit{ps}$ & $11.221\unit{ps}$ & $0.964$\\
        & $50\unit{mV}$ rms & $28.287\unit{ps}$ & $28.876\unit{ps}$ & $1.021$\\
\hline
\end{tabular}
\end{center}
...
is separately measured: $450.6\unit{MHz/V}$ on a $2.503\unit{ns}$ period
predicts $14.12\unit{ps}$ of period jitter for $5\unit{mV}$ rms, if the
disturbance held still across the cycle. The circuit gives
$2.861\unit{ps}$ --- a ratio of $0.203$, against the
$\sqrt{\tau/T} = \sqrt{100.1\unit{ps}/2503\unit{ps}} = 0.200$ that
averaging over the cycle predicts.

It holds across the decade rather than at one point: $0.203$, $0.206$ and
$0.200$ at the three judged amplitudes, where an oscillator that did not
average its input would sit at $1$. Three points within $3\%$ of a
parameter-free prediction, over a tenfold range of injected amplitude, is
not something a fit produces by accident. The model reproduces it because
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.3em]
  ...
  \item \textbf{Jitter TRANSFER is verified; intrinsic device noise is
        ...
        a tenfold range, worst ratio $0.964$ against a $0.25$ tolerance,
