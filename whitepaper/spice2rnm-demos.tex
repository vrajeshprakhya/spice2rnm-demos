...
moves $9.7\unit{\textmu V}$ rms within one oscillator period --- worth
$0.023\unit{ps}$ of movement against a $0.052\unit{ps}$ measurement floor.
...
closed-loop behaviour says it would measure something, when the netlist
declares a noise source of its own, or when you ask for it.
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.3em]
  ...
  \item \textbf{Jitter TRANSFER is verified; intrinsic device noise is
        ...
        worse than showing the number. At system level it is not measured
        at all, because the co-simulation boundary is sampled at
        $100\unit{ps}$ and the composed model's output crosses
        its threshold between samples, so a crossing can only be located
        to the nearest tick. That is a property of the probe, not of the
        model, and reporting it as jitter would be measuring the sampling
        grid.
