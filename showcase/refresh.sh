...
# Above this a comparison table is a trace, not a table: the waveform
# route writes one row per simulator timestep (18.6 MB for the filter),
# while the point-wise routes write one row per measured point (65 KB for
...
curate_equivalence() {  # run-dir, dest-name, [subdir ...]
  ...
  local man="$dst/MANIFEST.txt"
  {
    ...
    echo
    echo "PUBLISHED HERE: the logs of what ran, the per-point comparison"
    echo "tables the verdict is computed from, and the verdict itself."
  ...
  for sub in "$@"; do
    ...
    for f in $(cd "$d" && find . -type f | sort); do
      case "$f" in
        # Traces that happen to be named like evidence. boundary.log is
        # 116,018 co-simulation samples; a coverage DB is not a verdict.
        *boundary.log|*xezim_cov.json)
          printf '  %10s  %s\n' "$(stat -c %s "$d/$f")" "$sub/${f#./}" >> "$man" ;;
        # The comparison tables: one row per measured point, which is what
        # the verdict is computed from and what a reader would recompute
        # it from. Published whatever they are called.
        *_compare.csv|*compare.csv)
          if [ "$(stat -c %s "$d/$f")" -le "$COMPARE_MAX_BYTES" ]; then
...
def eq(e, label):
    # Each route reports the figures ITS check produces. The first version
    # printed rms/max/threshold for everything and rendered the phase
    # route as "rms=None max=None threshold=None" -- three absences where
    # the route has a perfectly good number, because the template was
    # written for the waveform route and applied to all of them.
    if not isinstance(e, dict):
        return
    if not e.get("checked"):
        print("%-22s NOT CHECKED  %s"
              % (label, "; ".join(e.get("errors") or []) or "")); return
    verdict = "PASS" if e.get("success") else "FAIL"
    metric = e.get("metric")
    if e.get("worst_error_s") is not None:
